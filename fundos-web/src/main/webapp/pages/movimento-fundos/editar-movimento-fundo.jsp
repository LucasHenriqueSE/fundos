...
<body>
	<form id="form1" name="form1" action="/fundos/movimentos/alterar"
		...
		<input type="hidden" name="fundo.id" value="${movimentoFundo.fundo.id}"/>
		<input type="hidden" name="tipoMovimento.id" value="${movimentoFundo.tipoMovimento.id}" />
		Valor: <input type="tel" name="valorMovimentoFundo" value="${movimentoFundo.valorMovimentoFundo}" />
			...
			Data: <input name="dataCadastro" value="<fmt:formatDate type="both" pattern="dd/MM/yyyy HH:mm:ss" dateStyle="short" timeStyle="medium" value="${movimentoFundo.dataMovimentoFundo}" />"/><br /> 
